...
import com.flyhz.framework.util.Taobao;
...
public class TaobaoDataImpl implements TaobaoData {
	...
	private void init() {
		if("1".equals(taobaoFlag)){//判断是否打开淘宝接口调用
			...
			appkey = Taobao.getAppKey();
			secret = Taobao.getAppSecret();
			sessionKey = Taobao.getAccessToken();
			sellerNick = Taobao.getSellerNick();
		}
	}
